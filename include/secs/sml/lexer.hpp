...
private:
    ...
    Token make_error(std::string_view message) noexcept {
        return make_error(lexer_errc::invalid_character, message);
    }

    [[nodiscard]] TokenType
    identifier_type(std::string_view text) const noexcept;
