...
class Runtime {
 public:
   ...
   */
  [[nodiscard]] const MessageDef* get_message(std::string_view name) const noexcept;

  /**
   * @brief 通过 Stream/Function 获取消息
   */
  [[nodiscard]] const MessageDef* get_message(std::uint8_t stream, std::uint8_t function) const noexcept;
   ...
   */
  [[nodiscard]] std::optional<std::string> match_response(
      std::uint8_t stream,
   ...
   * @brief 获取所有定时规则
   */
  [[nodiscard]] const std::vector<TimerRule>& timers() const noexcept { return document_.timers; }

  /**
   * @brief 获取所有消息定义
   */
  [[nodiscard]] const std::vector<MessageDef>& messages() const noexcept { return document_.messages; }

  /**
   * @brief 获取所有条件规则
   */
  [[nodiscard]] const std::vector<ConditionRule>& conditions() const noexcept { return document_.conditions; }
