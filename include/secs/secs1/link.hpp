 ...
 * - 由于 SECS-I 是半双工协议，强烈建议把对同一条 Link 的 read/write
 *   串行化（例如统一走 `asio::strand`）。否则多个线程/协程并发读写会造成：
 *   1) 字节交错（frame 被破坏）；
 *   2) 控制字节错配（把对端 ENQ/ACK/NAK 当成了自己在等待的响应）。
 */
class Link {
public:
    ...
    async_write(secs::core::bytes_view data) = 0;

    virtual asio::awaitable<std::pair<std::error_code, secs::core::byte>>
    async_read_byte(
 ...
 * - 支持简单的发送侧注入：固定延迟/丢弃前 N 个字节（用于模拟 T1/T2/NAK
 * 重传等场景）
 */
...
public:
    ...
    public:
        ...
        void
        set_fixed_delay(std::optional<secs::core::duration> delay) noexcept;
    ...
    private:
        ...
        Endpoint(asio::any_io_executor ex,
