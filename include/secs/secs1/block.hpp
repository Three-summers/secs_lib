 ...
 */
std::error_code decode_block(
  secs::core::bytes_view frame,
  DecodedBlock& out);
 ...
 */
std::vector<std::vector<secs::core::byte>> fragment_message(
  Header base_header,
  secs::core::bytes_view payload);
...
class Reassembler final {
 public:
  explicit Reassembler(std::optional<std::uint16_t> expected_device_id = std::nullopt);
