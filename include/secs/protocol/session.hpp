...
struct SessionOptions final {
  // T3：Reply timeout（协议层请求-响应匹配用）。
  secs::core::duration t3{std::chrono::seconds{45}};

  // run loop 的轮询间隔：用于 stop 检查与避免永久阻塞。
  secs::core::duration poll_interval{std::chrono::milliseconds{10}};
};

/**
 * @brief 协议层会话：统一 HSMS 与 SECS-I 的 send/request/run API。
 *
 * 能力：
 * - SystemBytes 分配与追踪（释放重用、wrap-around）
 * - 基于 SystemBytes + (Stream/Function) 的请求-响应匹配（T3）
 * - 根据 (Stream, Function) 路由入站 primary 到注册 handler，并在 W-bit=1 时自动回 secondary
 *
 * 说明：
 * - HSMS（全双工）推荐同时运行 async_run，用于接收并分发消息、唤醒 pending 请求。
