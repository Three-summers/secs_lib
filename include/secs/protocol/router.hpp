...
using Handler =
    std::function<asio::awaitable<HandlerResult>(const DataMessage &)>;
...
private:
    ...
    static constexpr Key make_key_(std::uint8_t stream,
                                   std::uint8_t function) noexcept {
        return static_cast<Key>((static_cast<Key>(stream) << 8U) |
                                static_cast<Key>(function));
    }
