...
/**
 * @brief SystemBytes 分配器：负责唯一性、在用追踪、释放重用与 wrap-around 处理。
 *
 * 说明：
 * - SystemBytes 在 HSMS/SECS-I 中用于请求-响应关联（response 回显 request 的 SystemBytes）。
 * - 本分配器仅保证“本端发出的消息”在 in_use 集合中的唯一性；不尝试与对端全局去重。
 * - 默认假设在同一执行器/线程语境下使用；若跨线程使用，本类内部使用 mutex 保护数据结构。
 */
class SystemBytes final {
 public:
   ...
   *
   * @return ok 成功；buffer_overflow 表示可用空间被耗尽（极端情况：2^32-1 全部在用）。
   */
