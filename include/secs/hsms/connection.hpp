...
class Stream {
 public:
  ...
  virtual asio::awaitable<std::pair<std::error_code, std::size_t>> async_read_some(
    core::mutable_bytes_view dst) = 0;
...
class Connection final {
 public:
  explicit Connection(asio::any_io_executor ex, ConnectionOptions options = {});
  explicit Connection(asio::ip::tcp::socket socket, ConnectionOptions options = {});
  explicit Connection(std::unique_ptr<Stream> stream, ConnectionOptions options = {});

  [[nodiscard]] asio::any_io_executor executor() const noexcept;
  [[nodiscard]] bool is_open() const noexcept;

  asio::awaitable<std::error_code> async_connect(const asio::ip::tcp::endpoint& endpoint);
 ...
 private:
  asio::awaitable<std::error_code> async_read_exactly(core::mutable_bytes_view dst);

  asio::awaitable<std::pair<std::error_code, std::size_t>> async_read_some_with_t8(
    core::byte* dst,
    std::size_t n);
