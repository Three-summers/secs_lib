...
static void bench_codec_deep_nested() {
    ...
    // 解码性能
    BENCH_RUN(
        "SECS-II: Decode deep nested list (64 levels)", encoded_bytes, 3, {
...
static void bench_codec_numeric_arrays() {
    ...
    // U4 数组
    {
        ...
        BENCH_RUN("SECS-II: U4 array encode (10000 items)",
                  array_size * sizeof(std::uint32_t),
                  3,
                  {
                      encoded.clear();
                      auto ec = encode(item, encoded);
    ...
    // F8 数组
    {
        ...
        BENCH_RUN("SECS-II: F8 array encode (10000 items)",
    ...
    // I8 数组
    {
        ...
        BENCH_RUN("SECS-II: I8 array encode (10000 items)",
                  array_size * sizeof(std::int64_t),
                  3,
                  {
                      encoded.clear();
                      auto ec = encode(item, encoded);
