...
static void bench_buffer_large_capacity() {
  ...
  BENCH_RUN("Buffer: 64MB expansion (1MB chunks)", target_size, 3, {
    buf.clear();
    for (std::size_t written = 0; written < target_size; written += chunk_size) {
...
static void bench_buffer_compact() {
  ...
  constexpr std::size_t buffer_size = 8 * 1024 * 1024;  // 8MB
  constexpr std::size_t chunk_size = 64 * 1024;         // 64KB
  constexpr std::size_t consume_size = 32 * 1024;       // 32KB
  ...
  BENCH_RUN("Buffer: Compact operations", buffer_size, 3, {
    ...
    // 填充到接近容量
    for (std::size_t written = 0; written < buffer_size; written += chunk_size) {
      auto ec = buf.append(bytes_view{chunk.data(), chunk.size()});
      if (ec) break;
    }
...
static void bench_buffer_writable_commit() {
  ...
  constexpr std::size_t total_size = 16 * 1024 * 1024;  // 16MB
