...
inline void print_results() {
  ...
  for (const auto& result : results()) {
    ...
    if (result.data_size >= 1024 * 1024) {
      ...
      std::cout << std::setw(15) << (std::to_string(result.data_size / 1024) + " KB");
    } else {
      std::cout << std::setw(15) << (std::to_string(result.data_size) + " B");
    }

    std::cout << std::fixed << std::setprecision(3)
              << std::setw(15) << result.elapsed_ms;
