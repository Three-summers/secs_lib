...
Message make_control_base(SType stype, std::uint16_t session_id, std::uint32_t system_bytes) {
  Message m;
...
Message make_deselect_req(std::uint16_t session_id, std::uint32_t system_bytes) {
...
Message make_linktest_req(std::uint16_t session_id, std::uint32_t system_bytes) {
...
Message make_separate_req(std::uint16_t session_id, std::uint32_t system_bytes) {
...
Message make_data_message(
  ...
  Message m;
  m.header.session_id = session_id;
  m.header.header_byte2 = static_cast<std::uint8_t>((w_bit ? 0x80U : 0x00U) | (stream & 0x7FU));
...
std::vector<core::byte> encode_frame(const Message& msg) {
  const std::uint32_t payload_len = static_cast<std::uint32_t>(kHeaderSize + msg.body.size());
...
std::error_code decode_payload(core::bytes_view payload, Message& out) noexcept {
  ...
  out.header = h;
  out.body.assign(payload.begin() + static_cast<std::ptrdiff_t>(kHeaderSize), payload.end());
  return {};
}
...
std::error_code decode_frame(core::bytes_view frame, Message& out, std::size_t& consumed) noexcept {
  ...
  const std::size_t total_len = static_cast<std::size_t>(kLengthFieldSize) + payload_len;
