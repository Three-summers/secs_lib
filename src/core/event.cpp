 ...
 *
 * - 等待者通过 asio::steady_timer 挂起；set()/cancel() 会对所有 waiter 执行
 * timer->cancel() 来“唤醒”。
 *
 * - 由于 timer->cancel() 与 timer 到期都会让 async_wait 返回，因此需要用两个
 * generation 计数区分来源：
...
void Event::reset() noexcept { signaled_ = false; }
...
Event::async_wait(std::optional<steady_clock::duration> timeout) {
    ...
    if (ec == asio::error::operation_aborted) {
        // 定时器被 cancel() 取消（set/cancel 都会 cancel 定时器）->
        // errc::cancelled
        co_return make_error_code(errc::cancelled);
    }
