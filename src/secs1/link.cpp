...
MemoryLink::Endpoint::Endpoint(asio::any_io_executor ex, std::shared_ptr<SharedState> shared, bool is_a)
...
void MemoryLink::Endpoint::set_fixed_delay(std::optional<secs::core::duration> delay) noexcept {
  if (!shared_) {
    return;
  }
  if (is_a_) {
...
asio::awaitable<std::error_code> MemoryLink::Endpoint::async_write(secs::core::bytes_view data) {
  if (!shared_) {
...
asio::awaitable<std::pair<std::error_code, secs::core::byte>> MemoryLink::Endpoint::async_read_byte(
  std::optional<secs::core::duration> timeout) {
  if (!shared_) {
    co_return std::pair{secs::core::make_error_code(secs::core::errc::invalid_argument), secs::core::byte{0}};
  }
...
std::pair<MemoryLink::Endpoint, MemoryLink::Endpoint> MemoryLink::create(asio::any_io_executor ex) {
