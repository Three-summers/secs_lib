...
std::error_code encode_block(
  ...
  out.push_back(static_cast<secs::core::byte>((header.reverse_bit ? 0x80 : 0x00) | dev_hi));
  out.push_back(dev_lo);

  out.push_back(static_cast<secs::core::byte>((header.wait_bit ? 0x80 : 0x00) | (header.stream & 0x7F)));
  out.push_back(static_cast<secs::core::byte>(header.function));

  const auto blk_hi = static_cast<secs::core::byte>((header.block_number >> 8) & 0x7F);
  const auto blk_lo = static_cast<secs::core::byte>(header.block_number & 0xFF);
  out.push_back(static_cast<secs::core::byte>((header.end_bit ? 0x80 : 0x00) | blk_hi));
  out.push_back(blk_lo);

  out.push_back(static_cast<secs::core::byte>((header.system_bytes >> 24) & 0xFF));
  out.push_back(static_cast<secs::core::byte>((header.system_bytes >> 16) & 0xFF));
  out.push_back(static_cast<secs::core::byte>((header.system_bytes >> 8) & 0xFF));
...
std::error_code decode_block(secs::core::bytes_view frame, DecodedBlock& out) {
  ...
  const auto cs_recv = static_cast<std::uint16_t>((static_cast<std::uint16_t>(frame[1 + length]) << 8) |
  ...
  header.device_id = static_cast<std::uint16_t>(((static_cast<std::uint16_t>(b1 & 0x7F)) << 8) |
  ...
  header.block_number = static_cast<std::uint16_t>(((static_cast<std::uint16_t>(b5 & 0x7F)) << 8) |
...
std::vector<std::vector<secs::core::byte>> fragment_message(Header base_header, secs::core::bytes_view payload) {
...
const Header& Reassembler::message_header() const noexcept {
  return header_;
}
...
std::error_code Reassembler::accept(const DecodedBlock& block) {
  if (!has_header_) {
    if (expected_device_id_.has_value() && block.header.device_id != *expected_device_id_) {
  ...
  if (block.header.stream != header_.stream || block.header.function != header_.function) {
    return make_error_code(errc::protocol_error);
  }
  if (block.header.reverse_bit != header_.reverse_bit || block.header.wait_bit != header_.wait_bit) {
    return make_error_code(errc::protocol_error);
  }
