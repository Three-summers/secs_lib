...
/*
 * SECS-II Item 的比较语义实现（对应 include/secs/ii/item.hpp）。
 *
 * 重点：
 * - 除浮点外，其它类型均可直接使用容器/值的 ==；
 * - 对于 F4/F8，选择“按位相等”而不是容差比较：
 *   - SECS-II 编解码以字节为单位，互通测试关注的是位模式是否一致；
 *   - 可正确处理 NaN、-0/+0 等边界情况（按值比较可能产生歧义）。
 *
 * 若你需要“业务意义上的近似相等”，应在业务层/规则层（例如 SML Runtime）单独实现。
 */
