...
class MemoryStream final : public Stream {
 public:
  ...
  asio::awaitable<std::pair<std::error_code, std::size_t>> async_read_some(
    secs::core::mutable_bytes_view dst) override {
    if (!inbox_) {
      co_return std::pair{make_error_code(errc::invalid_argument), std::size_t{0}};
    }

    while (inbox_->buf.empty()) {
      if (inbox_->closed) {
        co_return std::pair{std::make_error_code(std::errc::broken_pipe), std::size_t{0}};
      }
  ...
  asio::awaitable<std::error_code> async_connect(const asio::ip::tcp::endpoint&) override {
    co_return make_error_code(errc::invalid_argument);
  }
...
void test_message_encode_decode_roundtrip() {
  ...
  auto ec = secs::hsms::decode_frame(bytes_view{frame.data(), frame.size()}, out, consumed);
  TEST_EXPECT_OK(ec);
  ...
  bad[3] = 9;
  consumed = 0;
  ec = secs::hsms::decode_frame(bytes_view{bad.data(), bad.size()}, out, consumed);
  ...
  ec = secs::hsms::decode_payload(bytes_view{bad_ptype.data(), bad_ptype.size()}, out);
  TEST_EXPECT_EQ(ec, make_error_code(errc::invalid_argument));

  // decode_frame：payload_len > 16MB 上限（应立即拒绝，避免后续解析与等待更多数据）
  {
    ...
    ec = secs::hsms::decode_frame(bytes_view{len.data(), len.size()}, out, consumed);
...
void test_connection_loopback_framing() {
  ...
  Connection client_conn(std::move(duplex.client_stream), ConnectionOptions{.t8 = 50ms});
  Connection server_conn(std::move(duplex.server_stream), ConnectionOptions{.t8 = 50ms});
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT(std::equal(echo.body.begin(), echo.body.end(), msg.body.begin()));
...
void test_connection_t8_intercharacter_timeout() {
  ...
  Connection client_conn(std::move(duplex.client_stream), ConnectionOptions{.t8 = 10ms});
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      auto ec = co_await server_stream_ptr->async_write_all(bytes_view{frame.data(), first});
      TEST_EXPECT_OK(ec);

      asio::steady_timer delay(ioc);
      delay.expires_after(50ms);
      (void)co_await delay.async_wait(asio::as_tuple(asio::use_awaitable));
...
void test_connection_t8_disabled() {
  ...
  Connection client_conn(std::move(duplex.client_stream), ConnectionOptions{.t8 = secs::core::duration{}});
  Connection server_conn(std::move(duplex.server_stream), ConnectionOptions{.t8 = secs::core::duration{}});
...
void test_connection_null_stream_and_tcpstream_error_paths() {
  ...
  Connection tcp_conn(ioc.get_executor(), ConnectionOptions{.t8 = secs::core::duration{}});
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      ec = co_await null_conn.async_write_message(secs::hsms::make_select_req(0x0001, 0x1));
      TEST_EXPECT_EQ(ec, make_error_code(errc::invalid_argument));
...
void test_connection_async_read_message_invalid_frames() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      ec = co_await server_stream_ptr->async_write_all(bytes_view{frame.data(), frame.size()});
      TEST_EXPECT_OK(ec);

      std::tie(rec, std::ignore) = co_await client_conn.async_read_message();
      TEST_EXPECT_EQ(rec, make_error_code(errc::invalid_argument));
      ...
      // 3) payload_len > 16MB 上限
      {
        ...
        ec = co_await server_stream_ptr->async_write_all(bytes_view{len.data(), len.size()});
        TEST_EXPECT_OK(ec);
...
void test_connection_write_serialization_waiters() {
  ...
  auto slow_client_stream = std::make_unique<MemoryStream>(ioc.get_executor(), s2c, c2s, 10ms);
  Connection client_conn(std::move(slow_client_stream), ConnectionOptions{.t8 = secs::core::duration{}});

  std::atomic<int> finished{0};
  std::atomic<bool> done{false};

  auto send_one = [&](std::uint32_t sb) -> asio::awaitable<void> {
    auto ec = co_await client_conn.async_write_message(secs::hsms::make_select_req(0x0001, sb));
    TEST_EXPECT_OK(ec);
    ++finished;
    co_return;
  };
...
void test_session_select_and_linktest() {
  ...
  auto duplex = make_memory_duplex(ioc.get_executor());
  Connection client_conn(std::move(duplex.client_stream), ConnectionOptions{.t8 = opt.t8});
  Connection server_conn(std::move(duplex.server_stream), ConnectionOptions{.t8 = opt.t8});
...
void test_session_select_req_when_already_selected() {
  ...
  asio::co_spawn(ioc, server.async_open_passive(std::move(server_conn)), asio::detached);
...
void test_session_select_req_session_id_mismatch_disconnects() {
  ...
  auto duplex = make_memory_duplex(ioc.get_executor());
  Connection client_conn(std::move(duplex.client_stream), ConnectionOptions{.t8 = client_opt.t8});
  Connection server_conn(std::move(duplex.server_stream), ConnectionOptions{.t8 = server_opt.t8});
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT_EQ(server.state(), secs::hsms::SessionState::disconnected);
...
void test_session_deselect_req_disconnects_both_sides() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      ec = co_await client.async_send(secs::hsms::make_deselect_req(opt.session_id, client.allocate_system_bytes()));
      TEST_EXPECT_OK(ec);
...
void test_session_precondition_and_stop_branches() {
  ...
  auto duplex = make_memory_duplex(ioc.get_executor());
  Connection conn(std::move(duplex.client_stream), ConnectionOptions{.t8 = secs::core::duration{}});
  Connection peer(std::move(duplex.server_stream), ConnectionOptions{.t8 = secs::core::duration{}});
  (void)peer;
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      auto [rec, _] = co_await client.async_request_data(1, 1, bytes_view{body.data(), body.size()});
      TEST_EXPECT_EQ(rec, make_error_code(errc::invalid_argument));
...
void test_session_control_response_type_mismatch_times_out() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      ec1 = co_await server_conn.async_write_message(secs::hsms::make_select_rsp(0, m1.header.system_bytes));
      TEST_EXPECT_OK(ec1);
      ...
      (void)co_await server_conn.async_write_message(secs::hsms::make_select_rsp(0, m2.header.system_bytes));
      co_return;
    },
    asio::detached);
...
void test_session_pending_cancelled_on_disconnect() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      (void)co_await server_conn.async_write_message(secs::hsms::make_separate_req(opt.session_id, 0xABCDEF01));
...
void test_session_reopen_after_separate() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      // 触发断线
      auto ec = co_await server.async_send(
        secs::hsms::make_separate_req(opt.session_id, server.allocate_system_bytes()));
      TEST_EXPECT_OK(ec);
