...
class ScriptedLink final : public secs::secs1::Link {
 public:
  ...
  [[nodiscard]] asio::any_io_executor executor() const noexcept override { return ex_; }

  void push_read_ok(byte b) { reads_.push_back(ReadResult{std::error_code{}, b}); }
  void push_read_error(std::error_code ec) { reads_.push_back(ReadResult{ec, byte{0}}); }

  void push_write_error(std::error_code ec) { write_errors_.push_back(ec); }

  [[nodiscard]] const std::vector<std::vector<byte>>& writes() const noexcept { return writes_; }
  ...
  asio::awaitable<std::pair<std::error_code, byte>> async_read_byte(
    std::optional<secs::core::duration> timeout) override {
    (void)timeout;
    if (reads_.empty()) {
...
asio::awaitable<std::pair<std::error_code, std::vector<byte>>> read_frame(
  secs::secs1::Link& link,
...
void test_checksum_known() {
  ...
  TEST_EXPECT_EQ(secs::secs1::checksum(bytes_view{bytes.data(), bytes.size()}), 6);
}

void test_secs1_error_category_and_messages() {
  TEST_EXPECT_EQ(std::string_view(secs::secs1::error_category().name()), "secs.secs1");

  TEST_EXPECT_EQ(secs::secs1::make_error_code(secs::secs1::errc::invalid_block).message(), "invalid block");
  TEST_EXPECT_EQ(secs::secs1::make_error_code(secs::secs1::errc::checksum_mismatch).message(), "checksum mismatch");
  TEST_EXPECT_EQ(secs::secs1::make_error_code(secs::secs1::errc::device_id_mismatch).message(), "device id mismatch");
  TEST_EXPECT_EQ(secs::secs1::make_error_code(secs::secs1::errc::protocol_error).message(), "protocol error");
  TEST_EXPECT_EQ(secs::secs1::make_error_code(secs::secs1::errc::too_many_retries).message(), "too many retries");
  TEST_EXPECT_EQ(secs::secs1::make_error_code(secs::secs1::errc::block_sequence_error).message(), "block sequence error");
...
void test_encode_decode_roundtrip_single_block() {
  ...
  TEST_EXPECT_OK(secs::secs1::decode_block(bytes_view{frame.data(), frame.size()}, decoded));
  ...
  TEST_EXPECT_EQ(std::string_view(reinterpret_cast<const char*>(decoded.data.data()), decoded.data.size()), "hello");
}
...
void test_encode_data_too_large() {
  ...
  std::vector<byte> frame;
  auto ec = secs::secs1::encode_block(h, bytes_view{payload.data(), payload.size()}, frame);
  TEST_EXPECT_EQ(ec, make_error_code(errc::invalid_argument));
}
...
void test_decode_checksum_mismatch() {
  ...
  DecodedBlock decoded{};
  auto ec = secs::secs1::decode_block(bytes_view{frame.data(), frame.size()}, decoded);
  TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::checksum_mismatch));
}
...
void test_decode_invalid_too_small() {
  ...
  DecodedBlock decoded{};
  auto ec = secs::secs1::decode_block(bytes_view{frame.data(), frame.size()}, decoded);
  TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::invalid_block));
}

void test_decode_invalid_length_out_of_range() {
  std::vector<byte> frame(1 + secs::secs1::kHeaderSize + 2, 0);
  frame[0] = 254;  // > kMaxBlockLength
  DecodedBlock decoded{};
  auto ec = secs::secs1::decode_block(bytes_view{frame.data(), frame.size()}, decoded);
  TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::invalid_block));
}
...
void test_decode_block_frame_size_256_ok() {
  ...
  std::vector<byte> frame;
  TEST_EXPECT_OK(secs::secs1::encode_block(h, bytes_view{payload.data(), payload.size()}, frame));
...
void test_fragment_message_splits_and_decodes() {
  ...
  auto frames = secs::secs1::fragment_message(h, bytes_view{payload.data(), payload.size()});
  ...
  for (std::size_t i = 0; i < frames.size(); ++i) {
    DecodedBlock decoded{};
    TEST_EXPECT_OK(secs::secs1::decode_block(bytes_view{frames[i].data(), frames[i].size()}, decoded));
...
void test_fragment_message_empty_payload() {
  ...
  DecodedBlock decoded{};
  TEST_EXPECT_OK(secs::secs1::decode_block(bytes_view{frames[0].data(), frames[0].size()}, decoded));
  TEST_EXPECT(decoded.data.empty());
...
void test_reassembler_happy_path() {
  ...
  TEST_EXPECT(std::equal(out.begin(), out.end(), payload.begin(), payload.end()));
}
...
void test_reassembler_device_id_mismatch() {
  ...
  TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::device_id_mismatch));
}
...
void test_reassembler_sequence_error() {
  ...
  Reassembler re(h.device_id);
  auto ec = re.accept(decoded);
  TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::block_sequence_error));
}
...
void test_reassembler_mismatch_after_first_block() {
  ...
  DecodedBlock block1{};
  TEST_EXPECT_OK(secs::secs1::decode_block(bytes_view{frame1.data(), frame1.size()}, block1));
  ...
  // system_bytes 不一致
  {
    ...
    DecodedBlock block2{};
    TEST_EXPECT_OK(secs::secs1::decode_block(bytes_view{frame2.data(), frame2.size()}, block2));

    auto ec = re.accept(block2);
    TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::protocol_error));
  }
...
void test_memory_link_drop_next() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT_OK(co_await a.async_write(bytes_view{out.data(), out.size()}));
...
void test_memory_link_default_endpoint_invalid_argument() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      auto ec = co_await ep.async_write(bytes_view{out.data(), out.size()});
      TEST_EXPECT_EQ(ec, make_error_code(errc::invalid_argument));
...
void test_state_machine_send_receive_single_block() {
  ...
  asio::co_spawn(
    ioc,
    [&]() -> asio::awaitable<void> {
      auto ec = co_await sender.async_send(h, bytes_view{payload.data(), payload.size()});
      TEST_EXPECT_OK(ec);
      if (++done == 2) {
        watchdog.cancel();
        ioc.stop();
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT(std::equal(msg.body.begin(), msg.body.end(), payload.begin(), payload.end()));
      if (++done == 2) {
        watchdog.cancel();
        ioc.stop();
      }
...
void test_state_machine_send_unexpected_handshake_byte_protocol_error_scripted() {
  ...
  asio::co_spawn(
    ioc,
    [&]() -> asio::awaitable<void> {
      result = co_await sm.async_send(h, bytes_view{payload.data(), payload.size()});
      co_return;
    },
    asio::detached);

  ioc.run();
  TEST_EXPECT_EQ(result, secs::secs1::make_error_code(secs::secs1::errc::protocol_error));
}
...
void test_state_machine_send_block_timeout_retries_to_too_many_retries_scripted() {
  ...
  ioc.run();
  TEST_EXPECT_EQ(result, secs::secs1::make_error_code(secs::secs1::errc::too_many_retries));
  TEST_EXPECT_EQ(link.writes().size(), 3u);  // ENQ + block(第1次) + block(第2次)
}
...
void test_state_machine_receive_too_many_bad_blocks() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      auto [ec, _] = co_await receiver.async_receive(500ms);
      TEST_EXPECT_EQ(ec, secs::secs1::make_error_code(secs::secs1::errc::too_many_retries));
      if (++done == 2) {
        watchdog.cancel();
        ioc.stop();
      }
...
void test_state_machine_receive_device_id_mismatch() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT_OK(co_await a.async_write(bytes_view{frame.data(), frame.size()}));
      auto [ec1, resp] = co_await a.async_read_byte(200ms);
      TEST_EXPECT_OK(ec1);
      TEST_EXPECT_EQ(resp, secs::secs1::kNak);
...
void test_state_machine_receive_ignores_noise_before_enq() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT_OK(co_await a.async_write(bytes_view{noise.data(), noise.size()}));
...
void test_receiver_nak_then_accept_on_checksum_error() {
  ...
  std::vector<byte> bad;
  std::vector<byte> good;
  TEST_EXPECT_OK(secs::secs1::encode_block(h, bytes_view{payload.data(), payload.size()}, good));
  bad = good;
  bad[1 + secs::secs1::kHeaderSize] ^= 0xFF;
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      TEST_EXPECT_OK(co_await a.async_write(bytes_view{good.data(), good.size()}));
...
void test_t1_intercharacter_timeout() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      // 发剩余字节（此时接收侧应已超时退出）
      if (frame.size() > 2) {
        (void)co_await a.async_write(bytes_view{frame.data() + 2, frame.size() - 2});
      }
      co_return;
    },
    asio::detached);
...
void test_t3_reply_timeout() {
  ...
  asio::co_spawn(
    ioc,
    [&]() -> asio::awaitable<void> {
      auto [ec, _] = co_await host.async_transact(h, bytes_view{payload.data(), payload.size()});
      TEST_EXPECT_EQ(ec, make_error_code(errc::timeout));
      if (++done == 2) {
        watchdog.cancel();
        ioc.stop();
...
void test_t3_reply_success() {
  ...
  asio::co_spawn(
    ioc,
    [&]() -> asio::awaitable<void> {
      auto [ec, reply] = co_await host.async_transact(req, bytes_view{req_body.data(), req_body.size()});
      TEST_EXPECT_OK(ec);
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      auto send_ec = co_await equip.async_send(rep, bytes_view{reply_body.data(), reply_body.size()});
