...
class MemoryStream final : public Stream {
 public:
  MemoryStream(
    ...
    : ex_(std::move(ex)), inbox_(std::move(inbox)), outbox_(std::move(outbox)) {}
...
void test_router_set_find_erase_clear() {
  ...
  r.set(1, 1, [](const DataMessage&) -> asio::awaitable<secs::protocol::HandlerResult> {
    co_return secs::protocol::HandlerResult{std::error_code{}, std::vector<byte>{0xAA}};
  });
  TEST_EXPECT(r.find(1, 1).has_value());
  ...
  r.set(2, 3, [](const DataMessage&) -> asio::awaitable<secs::protocol::HandlerResult> {
    co_return secs::protocol::HandlerResult{std::error_code{}, std::vector<byte>{}};
  });
  r.clear();
...
void test_hsms_protocol_pending_filters() {
  ...
  secs::hsms::Session server(ioc.get_executor(), secs::hsms::SessionOptions{
  ...
  secs::hsms::Session client(ioc.get_executor(), secs::hsms::SessionOptions{
                                                 .session_id = session_id,
                                                 .t3 = 200ms,
                                                 .t5 = 10ms,
                                                 .t6 = 50ms,
  ...
  proto_server.router().set(1, 1, [&](const DataMessage& msg) -> asio::awaitable<secs::protocol::HandlerResult> {
    ...
    {
      const auto collide_primary = secs::hsms::make_data_message(
        session_id,
        3,
        3,
        true,
  ...
  asio::co_spawn(
    ioc,
    [&, server_conn = std::move(server_conn)]() mutable -> asio::awaitable<void> {
  ...
  asio::co_spawn(
    ioc,
    [&, client_conn = std::move(client_conn)]() mutable -> asio::awaitable<void> {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      auto [ec, rsp] = co_await proto_client.async_request(1, 1, bytes_view{payload.data(), payload.size()});
      TEST_EXPECT_OK(ec);
      TEST_EXPECT_EQ(rsp.stream, 1);
      TEST_EXPECT_EQ(rsp.function, 2);
      TEST_EXPECT(std::equal(rsp.body.begin(), rsp.body.end(), payload.begin()));
...
void test_hsms_protocol_run_without_poll_interval() {
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      auto [ec, rsp] = co_await proto_client.async_request(1, 1, as_bytes("once"));
      TEST_EXPECT_OK(ec);
      TEST_EXPECT_EQ(rsp.function, 2);
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      ...
      for (std::size_t i = 0; i < 1000; ++i) {
        std::vector<byte> payload = {static_cast<byte>(i & 0xFFU), static_cast<byte>((i >> 8U) & 0xFFU)};
        auto [ec, rsp] = co_await proto_client.async_request(1, 1, bytes_view{payload.data(), payload.size()});
        TEST_EXPECT_OK(ec);
        TEST_EXPECT_EQ(rsp.stream, 1);
        TEST_EXPECT_EQ(rsp.function, 2);
  ...
  asio::co_spawn(
    ...
    [&]() -> asio::awaitable<void> {
      for (std::size_t i = 0; i < 100; ++i) {
        ...
        auto [ec, rsp] = co_await proto_client.async_request(2, 3, bytes_view{payload.data(), payload.size()});
        TEST_EXPECT_OK(ec);
...
void test_session_invalid_arguments() {
  ...
  secs::hsms::Session client(ioc.get_executor(), secs::hsms::SessionOptions{.session_id = session_id, .auto_reconnect = false});
  Session proto_client(client, session_id, SessionOptions{.t3 = 10ms, .poll_interval = 1ms});

  asio::co_spawn(
    ioc,
    [&]() -> asio::awaitable<void> {
      auto [ec0, rsp0] = co_await proto_client.async_request(1, 0, as_bytes("x"));
      (void)rsp0;
      TEST_EXPECT_EQ(ec0, make_error_code(errc::invalid_argument));

      auto [ec1, rsp1] = co_await proto_client.async_request(1, 2, as_bytes("x"));
      (void)rsp1;
      TEST_EXPECT_EQ(ec1, make_error_code(errc::invalid_argument));

      auto [ec2, rsp2] = co_await proto_client.async_request(1, 0xFF, as_bytes("x"));
      (void)rsp2;
