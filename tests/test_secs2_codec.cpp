...
Item placeholder_item() {
  return Item::binary(std::vector<byte>{});
}
...
std::uint8_t header_length_bytes(const std::vector<byte>& encoded) {
  ...
  const auto length_bytes = static_cast<std::uint8_t>((encoded[0] & 0x03u) + 1u);
...
void test_roundtrip_all_types() {
  ...
  expect_roundtrip(Item::binary(std::vector<byte>{byte{0x00}, byte{0xFF}, byte{0x7E}}));
  ...
  expect_roundtrip(Item::i2(std::vector<std::int16_t>{0, 1, -1, 0x1234, -0x1234}));
  expect_roundtrip(Item::i4(std::vector<std::int32_t>{0, 1, -1, 0x1234567, -0x1234567}));
  expect_roundtrip(Item::i8(std::vector<std::int64_t>{0, 1, -1, 0x123456789LL, -0x123456789LL}));

  expect_roundtrip(Item::u1(std::vector<std::uint8_t>{0u, 1u, 255u}));
  expect_roundtrip(Item::u2(std::vector<std::uint16_t>{0u, 1u, 0x1234u, 0xFFFFu}));
  expect_roundtrip(Item::u4(std::vector<std::uint32_t>{0u, 1u, 0x12345678u, 0xFFFFFFFFu}));
  expect_roundtrip(Item::u8(std::vector<std::uint64_t>{0u, 1u, 0x123456789ABCDEF0ull, 0xFFFFFFFFFFFFFFFFull}));

  const float f_nan = std::bit_cast<float>(std::uint32_t{0x7FC00001u});
  const double d_nan = std::bit_cast<double>(std::uint64_t{0x7FF8000000000001ull});
...
void test_error_category_and_messages() {
  ...
  TEST_EXPECT_EQ(make_error_code(errc::truncated).message(), "truncated input");
  TEST_EXPECT_EQ(make_error_code(errc::invalid_header).message(), "invalid secs-ii header");
  TEST_EXPECT_EQ(make_error_code(errc::invalid_format).message(), "invalid secs-ii format code");
  TEST_EXPECT_EQ(make_error_code(errc::length_overflow).message(), "secs-ii length overflow");
  TEST_EXPECT_EQ(make_error_code(errc::length_mismatch).message(), "secs-ii length mismatch");
  TEST_EXPECT_EQ(make_error_code(errc::buffer_overflow).message(), "output buffer overflow");
...
void test_length_field_boundaries_ascii() {
  ...
  for (const auto& c : cases) {
    ...
    TEST_EXPECT_EQ(static_cast<std::size_t>(header_length_value(encoded)), c.size);
...
void test_stream_decode_consumed() {
  ...
  TEST_EXPECT_OK(decode_one(bytes_view{stream.data(), stream.size()}, out_a, consumed_a));
  TEST_EXPECT_EQ(consumed_a, a_size);
  TEST_EXPECT(out_a == a);

  Item out_b = placeholder_item();
  std::size_t consumed_b = 0;
  const auto tail = bytes_view{stream.data(), stream.size()}.subspan(consumed_a);
...
void test_decode_errors() {
  ...
  // 浮点长度不匹配（F4 长度=3）
  {
    const std::vector<byte> in{byte{0x84}, byte{0x03}, byte{0x00}, byte{0x00}, byte{0x00}};
    Item out = placeholder_item();
    std::size_t consumed = 0;
    const auto ec = decode_one(bytes_view{in.data(), in.size()}, out, consumed);
    TEST_EXPECT_EQ(ec, make_error_code(errc::length_mismatch));
...
void test_decode_deep_list_nesting_rejected() {
  ...
  for (std::size_t i = 0; i < depth; ++i) {
    in.push_back(byte{0x00});  // List, lenBytes=1
    in.push_back(byte{0x01});  // length=1（单子元素）
  }
  in.push_back(byte{0x20});  // Binary, lenBytes=1
...
void test_decode_large_ascii_length_truncated() {
  ...
  in.push_back(byte{0x42});  // ASCII, lenBytes=3
...
void test_length_overflow_limits() {
  // Boolean 的 length=values.size()，这里构造 length=kMaxLength+1 触发 length_overflow
  ...
  std::size_t size = 0;
  TEST_EXPECT_EQ(encoded_size(item, size), make_error_code(errc::length_overflow));
  ...
  std::vector<byte> buf;
  TEST_EXPECT_EQ(
    encode_to(secs::ii::mutable_bytes_view{buf.data(), buf.size()}, item, written),
  ...
  // ASCII 的 length=value.size()，同样覆盖 length_overflow 分支
  {
    ...
    TEST_EXPECT_EQ(encoded_size(ascii_item, ascii_size), make_error_code(errc::length_overflow));

    std::vector<byte> ascii_out;
    TEST_EXPECT_EQ(encode(ascii_item, ascii_out), make_error_code(errc::length_overflow));

    std::size_t ascii_written = 0;
    std::vector<byte> ascii_buf;
    TEST_EXPECT_EQ(
      encode_to(secs::ii::mutable_bytes_view{ascii_buf.data(), ascii_buf.size()}, ascii_item, ascii_written),
      make_error_code(errc::length_overflow));
  }
}
