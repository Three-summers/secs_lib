...
void test_lexer_error_category_messages() {
  ...
  TEST_EXPECT_EQ(make_error_code(lexer_errc::unterminated_string).message(), "unterminated string literal");
  TEST_EXPECT_EQ(make_error_code(lexer_errc::unterminated_comment).message(), "unterminated block comment");
  TEST_EXPECT_EQ(make_error_code(lexer_errc::invalid_character).message(), "invalid character");
  TEST_EXPECT_EQ(make_error_code(lexer_errc::invalid_hex_literal).message(), "invalid hexadecimal literal");
}
...
void test_parser_error_category_messages() {
  ...
  TEST_EXPECT_EQ(make_error_code(parser_errc::unexpected_token).message(), "unexpected token");
  TEST_EXPECT_EQ(make_error_code(parser_errc::expected_item).message(), "expected item");
  TEST_EXPECT_EQ(make_error_code(parser_errc::expected_identifier).message(), "expected identifier");
  TEST_EXPECT_EQ(make_error_code(parser_errc::expected_number).message(), "expected number");
  TEST_EXPECT_EQ(make_error_code(parser_errc::invalid_stream_function).message(), "invalid stream/function format");
  TEST_EXPECT_EQ(make_error_code(parser_errc::unclosed_item).message(), "unclosed item");
  TEST_EXPECT_EQ(make_error_code(parser_errc::invalid_condition).message(), "invalid condition");
}

void test_lexer_unterminated_block_comment_is_error() {
  Lexer lexer("/* not closed");
  auto result = lexer.tokenize();
  TEST_EXPECT_EQ(result.ec, make_error_code(lexer_errc::unterminated_comment));
...
void test_lexer_unterminated_string_is_error() {
  {
    Lexer lexer("\"abc");
    auto result = lexer.tokenize();
    TEST_EXPECT_EQ(result.ec, make_error_code(lexer_errc::unterminated_string));
    TEST_EXPECT_EQ(result.error_line, 1u);
    TEST_EXPECT_EQ(result.error_column, 1u);
  }
  {
...
void test_parser_more_syntax_error_branches() {
  ...
  {
    auto result = parse_sml(".");
    TEST_EXPECT_EQ(result.ec, make_error_code(parser_errc::unexpected_token));
  }
  ...
  // 3) SxFy 格式错误：触发 invalid_stream_function
  {
    ...
    TEST_EXPECT_EQ(result.ec, make_error_code(parser_errc::invalid_stream_function));
  }
...
void test_runtime_condition_index_and_expected_matching() {
  ...
  // 3) 元素不相等 -> 不匹配
  TEST_EXPECT(!rt.match_response(1, 1, Item::list({Item::ascii("BAD"), Item::ascii("OK")})).has_value());
...
void test_runtime_condition_name_lookup_paths() {
  ...
  // message_name="S128F1" 会因 stream 超范围导致按“名称”查找，仍应能命中规则。
...
void test_runtime_float_tolerance_match() {
  ...
  // 误差在容差内 -> 命中
  auto ok_rsp = rt.match_response(1, 1, Item::list({Item::f4(std::vector<float>{1.00005f})}));
  TEST_EXPECT(ok_rsp.has_value());

  // 误差过大 -> 不命中
  auto bad_rsp = rt.match_response(1, 1, Item::list({Item::f4(std::vector<float>{1.1f})}));
