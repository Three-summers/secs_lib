...
void test_append_consume_basic() {
    ...
    TEST_EXPECT_EQ(std::string_view(reinterpret_cast<const char *>(
                                        buf.readable_bytes().data()),
...
void test_compact() {
    ...
    TEST_EXPECT_EQ(std::string_view(reinterpret_cast<const char *>(out.data()),
                                    out.size()),
                   "efWXYZ");
}
...
void test_max_capacity_append_rejects() {
    ...
    auto ec = buf.append(as_bytes("56789")); // 总长度=9 > 上限=8
    TEST_EXPECT_EQ(ec, make_error_code(errc::buffer_overflow));
}
...
void test_compact_then_grow() {
    ...
    TEST_EXPECT_OK(
        buf.append(as_bytes("abcde"))); // compact（整理）后仍不足，触发扩容
    auto out = buf.readable_bytes();
    TEST_EXPECT_EQ(out.size(), 9u);
    TEST_EXPECT_EQ(std::string_view(reinterpret_cast<const char *>(out.data()),
                                    out.size()),
...
void test_heap_grow_preserves_data() {
    ...
    TEST_EXPECT_OK(
        buf.append(as_bytes("b"))); // 触发从内联缓冲区到堆缓冲区的扩容
