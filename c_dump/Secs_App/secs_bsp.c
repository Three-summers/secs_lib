...
*/
int Secs_SerialSend(SecsBSPInfo *Secs_BSPInfo,
                    uint8_t *send_buf,
                    uint16_t buf_size) {
#ifdef __linux__
    serial_listener_write(
        Secs_BSPInfo->Secs_Serial, (char *)send_buf, (size_t)buf_size);
#else
...
*/
int Secs_TimerCtrl(SecsBSPInfo *Secs_BSPInfo,
                   uint8_t SecsTimer_en,
                   uint16_t Secs_IntrTime) {
#ifdef __linux__
    if (SecsTimer_en == 0)
...
#else
    if (SecsTimer_en == 1) {
        __HAL_TIM_SET_AUTORELOAD(Secs_BSPInfo->Secs_Timer,
                                 Secs_IntrTime * 1000 - 1);
...
void SecsTimer_IntrCallback1(void) { Secs_RecvInfo1.SecsTimeout_Flag = 1; }
...
void SecsSerial_RecvCallback2(const char *Serial_RecvBuf, size_t RecvBuf_Size) {
    ...
    if (SecsRecv_BufSize + RecvBuf_Size <= 256) {
        memcpy(Secs_RecvInfo3.SecsRecv_Buf + SecsRecv_BufSize,
               Serial_RecvBuf,
               RecvBuf_Size);
...
void SecsTimer_IntrCallback2(void) { Secs_RecvInfo2.SecsTimeout_Flag = 1; }
...
void SecsTimer_IntrCallback3(void) {
    if (Secs_RecvInfo2.SecsRecv_Flag) {
        ...
        if (Secs_RecvInfo2.SecsRecv_Buf) {
            memcpy(Secs_RecvInfo2.SecsRecv_Buf,
                   Secs_RecvInfo3.SecsRecv_Buf,
                   SecsRecv_BufSize);
...
int init_secs_bsp(char *Secs_Serial_Device1, char *Secs_Serial_Device2) {

    int secs_init_result = 0;

    // create serial
    serial_listener_t *Secs_Serial1 =
        (serial_listener_t *)malloc(sizeof(serial_listener_t));
    if (Secs_Serial1 == NULL) {
        printf("[error ] failed to allocate memory for Secs_Serial1\n");
        return -1;
    }

    secs_init_result = serial_listener_init(Secs_Serial1,
                                            "Secs_Serial1",
                                            Secs_Serial_Device1,
                                            SECE_UART_BAUDRATE,
                                            SecsSerial_RecvCallback1);
    ...
    // create timer
    timer_thread_t *Secs_Timer1 =
        (timer_thread_t *)malloc(sizeof(timer_thread_t));
    if (Secs_Timer1 == NULL) {
        printf("[error ] failed to allocate memory for Secs_Timer1\n");
        free(Secs_Serial1);
        return -1;
    }
    secs_init_result = timer_thread_create(
        Secs_Timer1, "Secs_Timer1", SecsTimer_IntrCallback1);
    if (secs_init_result < 0) {
    ...
    // create serial
    serial_listener_t *Secs_Serial2 =
        (serial_listener_t *)malloc(sizeof(serial_listener_t));
    ...
    secs_init_result = serial_listener_init(Secs_Serial2,
                                            "Secs_Serial2",
                                            Secs_Serial_Device2,
                                            SECE_UART_BAUDRATE,
                                            SecsSerial_RecvCallback2);
    if (secs_init_result < 0) {
        perror("[error ] create SECS serial port data receiving thread");
        return -1;
    }
    ...
    secs_init_result = timer_thread_create(
        Secs_Timer2, "Secs_Timer2", SecsTimer_IntrCallback2);
    if (secs_init_result < 0) {
    ...
    secs_init_result = timer_thread_create(
        Secs_Timer3, "Secs_Timer3", SecsTimer_IntrCallback3);
    if (secs_init_result < 0) {
...
int init_secs_bsp(UART_HandleTypeDef *HalUart, TIM_HandleTypeDef *HalTimer) {
