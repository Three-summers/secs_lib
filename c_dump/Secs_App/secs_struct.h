...
typedef struct {
    uint8_t  SendData_Ctrl; // secs 报文控制，若为1，则表明已经转化成串口可发送的形式，若为0，则需要进一步转换
    uint8_t* SendData_Buf;  // secs 发送的数据，已指针的形式，可以根据发送数据包打大小而调整
    size_t   SendData_Size; // secs 发送数据包的整个长度
    uint16_t SendData_Ptr;  // secs 发送数据包的指针，拆包发送时可用
} SendDataInfo;

typedef struct {
    uint8_t  SystemByte_Ctrl;       // 0:生成串口发送报文时，system byte需要递增；1：system byte不变，并使用SystemByte_Maintain
    uint32_t SystemByte_Maintain;   // secs 回复报文时system byte需相同，以及发送大包拆成多包时，system byte也需相同
