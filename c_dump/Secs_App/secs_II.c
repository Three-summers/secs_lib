...
void SecsII_AddByte(SendDataInfo* SecsArray, uint8_t value) {
    SecsArray->SendData_Buf = realloc(SecsArray->SendData_Buf, SecsArray->SendData_Size + 1);
    SecsArray->SendData_Buf[SecsArray->SendData_Size++] = value;
}

int SecsII_AddUint(SendDataInfo* SecsArray, uint8_t ByteNum, uint32_t UintValue) {
    switch (ByteNum) {
        case 0x01:
        ...
        case 0x02:
            ...
            for (int i = 0; i < ByteNum; i++) {
                SecsII_AddByte(SecsArray, (UintValue_trans >> (8 * (1-i))) & 0xFF);
            }
            break;
        case 0x04:
...
int SecsII_AddFloat(SendDataInfo* SecsArray, uint8_t ByteNum, float FloatValue) {
    switch (ByteNum) {
        case 0x04:
        ;
            union {
...
int parse_number(char** ptr, uint8_t* num) {
    ...
    // 错误检查：无数字/溢出/超出范围
    if (end == str) return -1;         // 无有效数字
    if (errno == ERANGE) return -1;    // 数值溢出
    if (val < 0 || val > 0xFF) return -1; // 超出uint8_t范围
...
int parse_unsigned(char** ptr, uint32_t* num) {
    ...
    if (end == str) return -1;          // 无有效数字
    if (errno == ERANGE) return -1;     // 数值溢出
    if (val > UINT32_MAX) return -1;    // 超出uint32_t范围
...
int Secs_MessageArrange(char *Secs_string, SendDataInfo *SecsArray) {
    ...
    while (*Secs_string) {
        ...
        */
            ...
            Secs_string += 3;
            // 跳过空白字符
            while (isspace(*Secs_string)) Secs_string++;
        ...
        */
        else if ((strncmp(Secs_string, "<U", 2) == 0) || (strncmp(Secs_string, "<u", 2) == 0)) {
            Secs_string += 2;
            uint8_t UintNum;
        ...
        */
        else if ((strncmp(Secs_string, "<I", 2) == 0) || (strncmp(Secs_string, "<i", 2) == 0)) {
            Secs_string += 2;
            uint8_t IntNum;
...
void SecsII_PrintArray(SendDataInfo *SecsArray) {
    ...
    for (size_t i = 0; i < SecsArray->SendData_Size; i++) {
        ...
        if (i != SecsArray->SendData_Size - 1) printf(", ");
        if ((i + 1) % 8 == 0) printf("\n ");
    }
    printf("};\n");
}
