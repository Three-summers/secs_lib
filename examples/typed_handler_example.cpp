...
public:
    ...
    handle([[maybe_unused]] const S1F1Request &request,
...
public:
    asio::awaitable<std::pair<std::error_code, S2F14Response>>
    handle(const S2F13Request &request, const DataMessage &raw) override {
        std::cout << "[S2F13Handler] Received S" << static_cast<int>(raw.stream)
                  << "F" << static_cast<int>(raw.function) << " requesting "
                  << request.ecids.size() << " ECIDs\n";
